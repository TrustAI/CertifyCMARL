...
def main(env_name, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes, max_epsilon, min_epsilon,
    ...
    for episode_i in range(max_episodes):
        ...
        if episode_i == max_episodes-1:
            torch.save(q, 'vdn'+ env_name)
    
    #q_new = torch.load('vdn')
    #test_score = test(test_env, test_episodes, q_new)
    #print(test_score)
    env.close()
    test_env.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', required=False, default='ma_gym:TrafficJunction10-v0')
    parser.add_argument('--seed', type=int, default=1, required=False)
    parser.add_argument('--no_recurrent', action='store_true')
    parser.add_argument('--max_episodes', type=int, default=20000, required=False)
