...
def train(q, q_target, mix_net, mix_net_target, memory, optimizer, gamma, batch_size, update_iter=10, chunk_size=10,
          perturb = 'False',grad_clip_norm=5,sigma= 0.1):
...
def main(env_name, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes,
         ...
         update_target_interval, recurrent,perturb):
    # create env.
    ...
    for episode_i in range(max_episodes):
        ...
        epsilon = torch.from_numpy(np.array((max(min_epsilon, max_epsilon - (max_epsilon - min_epsilon) * (episode_i / (0.6 * max_episodes))))))
        state = env.reset()
        done = [False for _ in range(env.n_agents)]
        with torch.no_grad():
            hidden = q.init_hidden()
        ...
        if memory.size() > warm_up_steps:
            train(q, q_target, mix_net, mix_net_target, memory, optimizer, gamma, batch_size, update_iter, chunk_size,perturb)
        ...
        if episode_i % log_interval == 0 and episode_i != 0:
            ...
            print("#{:<10}/{} episodes , avg train score : {:.1f}, test score: {:.1f} n_buffer : {}, eps : {:.1f},time : {:.1f}"
                  .format(episode_i, max_episodes, train_score, test_score, memory.size(), epsilon,time.time()-start_time))
            if USE_WANDB:
                wandb.log({'episode': episode_i, 'test-score': test_score,
                           'buffer-size': memory.size(), 'epsilon': epsilon, 'train-score': train_score})
            score = 0
        if episode_i == max_episodes-1:
            torch.save(q, 'qmix_TrafficJunction4-v0')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--perturn_obs', type=str, default='False', required=False)
    # Process arguments
